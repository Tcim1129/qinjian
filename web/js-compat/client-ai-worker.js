(function() {
  ...
  function summarizePii(text) {
    const phoneHits = collectMatches(PHONE_REGEX, text, "phone").filter((item) => isStandaloneNumberMatch(text, item.index, String(item.match || "").length));
    const hits = [
      ...phoneHits,
  ...
  function redactText(text) {
    return text.replace(PHONE_REGEX, (match, prefix, suffix, offset, source) => isStandaloneNumberMatch(source, offset, match.length) ? `${prefix}****${suffix}` : match).replace(EMAIL_REGEX, "$1***@$2").replace(NAME_REGEX, "$1[姓名已隐藏]").replace(ADDRESS_HINT_REGEX, "$1：[位置已隐藏]").replace(/\bwxid_[a-zA-Z0-9_-]{5,}\b/g, "[微信号已隐藏]").replace(/\b([a-zA-Z][-_a-zA-Z0-9]{5,19})\b/g, (token) => {
  ...
  function buildClientTags(text, intent, riskLevel, piiSummary, sentiment) {
    ...
    if (((piiSummary == null ? void 0 : piiSummary.total_hits) || 0) > 0)
      tags.push("contains_pii");
    if (sentiment !== "neutral")
      tags.push(`sentiment:${sentiment}`);
    if (/截图|聊天记录|微信|消息/.test(text))
      tags.push("message_context");
    if (/异地|距离|见面/.test(text))
      tags.push("distance_context");
    return Array.from(new Set(tags));
  }
  function precheckText(payload) {
    const text = String((payload == null ? void 0 : payload.text) || "").trim();
    const privacyMode = (payload == null ? void 0 : payload.privacy_mode) === "local_first" ? "local_first" : "cloud";
    const aiAssistEnabled = (payload == null ? void 0 : payload.ai_assist_enabled) !== false;
    const pii = summarizePii(text);
    const risk = detectRisk(text);
    const sentiment = aiAssistEnabled ? detectSentiment(text) : "neutral";
    const intent = aiAssistEnabled ? classifyIntent(text, risk.level) : risk.level === "high" ? "crisis" : "daily";
    ...
    return {
      source_type: (payload == null ? void 0 : payload.source_type) || "text",
      intent,
      risk_level: risk.level,
      risk_hits: risk.hits,
      pii_summary: pii.summary,
      privacy_mode: privacyMode,
      upload_policy: (payload == null ? void 0 : payload.upload_policy) || "full",
      redacted_text: redactedText,
      client_tags: clientTags,
      device_meta: (payload == null ? void 0 : payload.device_meta) || null,
      ai_assist_enabled: aiAssistEnabled,
      sentiment_hint: sentiment
    };
  }
  function describeAttachmentMeta(payload) {
    const sourceType = (payload == null ? void 0 : payload.source_type) || "mixed";
    const meta = (payload == null ? void 0 : payload.device_meta) || {};
    const aiAssistEnabled = (payload == null ? void 0 : payload.ai_assist_enabled) !== false;
    const clientTags = [];
    if (aiAssistEnabled && sourceType === "image") {
      if ((meta.width || 0) >= 1e3)
        clientTags.push("high_resolution_image");
    ...
    if (aiAssistEnabled && sourceType === "voice") {
      if ((meta.duration_seconds || 0) < 2)
        clientTags.push("short_voice");
      if ((meta.silence_ratio || 0) > 0.55)
        clientTags.push("mostly_silent_voice");
    }
    return {
      ...
      privacy_mode: (payload == null ? void 0 : payload.privacy_mode) === "local_first" ? "local_first" : "cloud",
      upload_policy: (payload == null ? void 0 : payload.upload_policy) || "full",
      redacted_text: null,
      client_tags: clientTags,
      device_meta: meta,
  ...
  self.onmessage = (event) => {
    ...
    try {
      ...
      self.postMessage({
        id,
        ok: false,
        error: (error == null ? void 0 : error.message) || "worker_failed"
      });
    }
  };
})();
