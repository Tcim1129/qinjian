...
REMOTE_HOST = "143.198.110.145"
REMOTE_USER = "root"
REMOTE_PASSWORD = "7507906119HJL"
REMOTE_ROOT = "/opt/qinjian"


def http_json(method: str, path: str, token: str | None = None, payload: dict | None = None, query: dict | None = None, timeout: int = 180):
    url = f"{BASE_URL}{path}"
    if query:
...
def poll_latest_report(token: str, pair_id: str, report_type: str, timeout_seconds: int = 420):
...
def set_latest_daily_dates(pair_id: str, target_date: str) -> None:
    run_remote_sql(
        ...
        "WHERE id = (SELECT id FROM reports WHERE pair_id = '{pair_id}' AND type = 'DAILY' ORDER BY created_at DESC LIMIT 1);"
        .format(pair_id=pair_id, target_date=target_date)
    )
...
def set_latest_weekly_date(pair_id: str, target_date: str) -> None:
    run_remote_sql(
        "UPDATE reports SET report_date = DATE '{target_date}' "
        "WHERE id = (SELECT id FROM reports WHERE pair_id = '{pair_id}' AND type = 'WEEKLY' ORDER BY created_at DESC LIMIT 1);"
        .format(pair_id=pair_id, target_date=target_date)
    )


def seed_daily_reports(pair_id: str, start_index: int, day_specs: list[tuple[str, int, str]]) -> None:
    values = []
    for offset, (target_date, health_score, insight) in enumerate(day_specs, start=start_index):
        report_id = str(uuid4())
        payload = json.dumps(
            {
...
def main() -> int:
    ...
    print("[1/7] 注册并登录测试账号")
    http_json("POST", "/auth/register", payload={"email": email_a, "nickname": f"周期A{suffix[-4:]}", "password": password})
    http_json("POST", "/auth/register", payload={"email": email_b, "nickname": f"周期B{suffix[-4:]}", "password": password})
    token_a = http_json("POST", "/auth/login", payload={"email": email_a, "password": password})["access_token"]
    token_b = http_json("POST", "/auth/login", payload={"email": email_b, "password": password})["access_token"]

    print("[2/7] 创建配对")
    pair = http_json("POST", "/pairs/create", token=token_a, payload={"type": "couple"})
    pair_id = pair["id"]
    http_json("POST", "/pairs/join", token=token_b, payload={"invite_code": pair["invite_code"]})
    ...
    http_json("POST", "/reports/generate-weekly", token=token_a, query={"pair_id": pair_id})
    ...
    print("[7/7] 生成月报")
    http_json("POST", "/reports/generate-monthly", token=token_a, query={"pair_id": pair_id})
    monthly = poll_latest_report(token_a, pair_id, "monthly")
    if monthly.get("status") != "completed":
        raise RuntimeError(f"月报生成失败: {monthly}")

    weekly_history = http_json("GET", "/reports/history", token=token_a, query={"pair_id": pair_id, "report_type": "weekly", "limit": 5})
    monthly_latest = http_json("GET", "/reports/latest", token=token_a, query={"pair_id": pair_id, "report_type": "monthly"})
    ...
    summary = {
        ...
        "weekly_reports": [
            {
                "report_date": weekly_history[0].get("report_date") if len(weekly_history) > 0 else None,
                "status": weekly_history[0].get("status") if len(weekly_history) > 0 else None,
                "health_score": weekly_history[0].get("health_score") if len(weekly_history) > 0 else None,
            },
            {
                "report_date": weekly_history[1].get("report_date") if len(weekly_history) > 1 else None,
                "status": weekly_history[1].get("status") if len(weekly_history) > 1 else None,
                "health_score": weekly_history[1].get("health_score") if len(weekly_history) > 1 else None,
            },
        ],
        "monthly_report": {
