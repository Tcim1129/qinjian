...
import { loadCachedSession } from '@app/utils/session.js'

export function createQinjianStore() {
  const store = createStore({
    ...
    mutations: {
      ...
      SET_PAIR_SUMMARY(state, payload) {
        ...
        state.pairInfo =
          payload?.is_paired && payload?.active_pair?.status === 'active'
            ? payload.active_pair
            : null
      },
    ...
    actions: {
      async syncUserAndPair({ commit }) {
        ...
        const [me, summary] = await Promise.all([api.getMe(), api.getPairSummary()])
