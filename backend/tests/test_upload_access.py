...
def test_build_upload_access_url_prefers_dedicated_signing_key():
    original_secret = settings.SECRET_KEY
    original_upload_key = settings.UPLOAD_SIGNING_KEY
    settings.SECRET_KEY = "0123456789abcdef0123456789abcdef"
    settings.UPLOAD_SIGNING_KEY = "fedcba9876543210fedcba9876543210"
    try:
        access_url = build_upload_access_url("/uploads/images/private-key.png")
        parsed = urlparse(access_url)
        query = parse_qs(parsed.query)
        expires = int(query["expires"][0])
        signature = query["sig"][0]

        assert verify_upload_access("/uploads/images/private-key.png", expires, signature) is True

        settings.UPLOAD_SIGNING_KEY = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"

        assert verify_upload_access("/uploads/images/private-key.png", expires, signature) is False
    finally:
        settings.SECRET_KEY = original_secret
        settings.UPLOAD_SIGNING_KEY = original_upload_key
